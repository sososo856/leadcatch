# LeadCatch AI — Pricing Strategy
# Purpose: Competitive pricing, tiers, and offer ladder
# Date: 2026-04-16

---

## Competitor Analysis

| Competitor | Price | What They Offer |
|---|---|---|
| Hatch (usehatchapp.com) | $499/mo | AI texting for home services, multi-channel outreach, CRM integration |
| Podium | $399/mo | Webchat, text marketing, review management, payments |
| Calldrip | $99/mo per user | Speed-to-lead, instant callback, lead routing |
| ServiceTitan Marketing Pro | $399/mo (bundled) | Call tracking, automated follow-up, reputation management |
| Housecall Pro | $149/mo (Max plan) | Automated follow-ups, online booking, review requests |

**Cheapest comparable**: Calldrip at $99/mo (limited to callback only — no AI qualification, no booking, no follow-up sequences).

**Most comparable**: Hatch at $499/mo (closest feature overlap — AI texting, qualification, multi-channel).

**LeadCatch pricing rationale**: We price 15–25% above comparable competitors because we deliver a fully managed, done-for-you system. Hatch requires the contractor to manage campaigns. Podium requires staff to respond. LeadCatch runs itself.

---

## LeadCatch AI — $2,997/mo

**One plan. Everything included. No tiers.**

**For:** Roofing contractors doing $750K–$2.5M+/year with 15–50+ leads per week.

**Includes:**
- Alex AI assistant (SMS + email)
- 60-second missed call response
- 7-touch follow-up sequence over 14 days
- Google Calendar booking integration
- Unlimited lead sources (LSA, Angi, HomeAdvisor, website, ads)
- Storm Surge (proactive outreach to storm-affected zip codes)
- Automated Google review requests after job completion
- Dedicated 60-minute onboarding call
- Weekly performance dashboard
- Up to 500 lead conversations/month
- Priority support (Slack or text)

---

## Founder's Lifetime Deal

**Price:** $14,970 one-time (~58% off annual plan: $2,997 x 12 = $35,964 → discounted to $14,970 = 5x monthly rate)

**What they get:** LeadCatch AI plan locked in for life. No monthly payments. No price increases. Ever.

**Cap:** 50 seats. Once they're gone, they're gone. Counter displayed on landing page.

**Why:** Creates urgency. Early adopters get rewarded. Generates upfront cash to fund growth.

**Fine print:** Lifetime = as long as LeadCatch operates. Covers up to 500 conversations/month. Overages billed at $0.15/conversation.

---

## DFY Installation Add-On — $497

**What it includes:**
- Full audit of the contractor's current lead sources and response process
- Connection of all lead sources (website, LSA, Angi, HomeAdvisor, Facebook)
- Google Calendar sync and availability setup
- Alex persona customization (company name, tone, service area, FAQ training)
- Test conversation verification (3 test leads run through the system)
- 30-minute walkthrough call showing the client exactly how it works
- Delivered within 48 hours of payment

**Who it's for:** Any client who wants zero setup friction.

---

## Price Ladder (Full Offer Stack)

| Price | Offer | Purpose |
|---|---|---|
| $0 | Lead Magnet: "Missed Call Revenue Calculator" | Capture email, demonstrate the problem |
| $27 | Tripwire: "Lead Response Audit + 5 SMS Templates" | Micro-commitment, prove value, enter buyer pool |
| $97 | Mini-Course: "Speed-to-Lead Masterclass for Contractors" | Education-based conversion, positions LeadCatch as authority |
| $497 | DFY Installation Add-On | Zero setup friction, fully configured in 48 hours |
| $997 | LeadCatch AI (first month at 67% discount) | Anchor the $2,997 price, get them into the system |

**Conversion logic:** Every rung warms the buyer for the next. A contractor who pays $27 is 7x more likely to pay $297. A contractor who pays $297 is already inside the system and seeing results — converting to $2,997/mo becomes a no-brainer.
